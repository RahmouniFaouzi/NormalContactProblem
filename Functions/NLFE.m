function NLFE(ITRA,TOL,ATOL,TIMS,NOUT,PROP,SDISPT,XYZ,LE,ELXYM,Tool_Ad,OMEGAN, PlotR)
%***********************************************************************
% MAIN PROGRAM FOR HYPERELASTIC/ELASTOPLASTIC ANALYSIS
%***********************************************************************
%%
global DISPDD DISPTD FORCE GKF DISPPD CNTFORCE	SIGMA % Global variables
%
...
if nargin < 13 || PlotR == 0
    fprintf('\n-----> The maximum contact force is: %.6f N\n', max(CNTFORCE));
    fprintf('-----> The maximum Stress is: %.6f\n\n', max(max(SIGMA)));
end

% Plot Results
if nargin == 13
    coord = XYZ + reshape(DISPTD,2,length(DISPTD)/2)';
    plot_stress_field(coord, LE, SIGMA, 'xx', PlotR, 1, ELXYM)
    hold off
end
